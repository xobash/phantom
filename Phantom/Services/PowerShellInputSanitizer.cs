...
public static class PowerShellInputSanitizer
{
    ...
    public static string EnsureServiceName(string? value, string context)
    {
        var trimmed = (value ?? string.Empty).Trim();
        if (trimmed.Length == 0)
        {
            throw new ArgumentException($"{context}: service name is required.");
        }

        if (!ServiceNameRegex.IsMatch(trimmed))
        {
            throw new ArgumentException($"{context}: invalid service name '{trimmed}'.");
        }
